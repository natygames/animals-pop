...
import com.nativegame.animalspop.prize.Prize;
import com.nativegame.animalspop.prize.PrizeList;
...
public class WheelDialog extends GameDialog implements View.OnClickListener,
    ...
    private final PrizeList mPrizeList;
    ...
    public WheelDialog(GameActivity activity, WheelTimer wheelTimer) {
        ...
        setExitAnimationResId(R.anim.exit_disappear);
        mPrizeList = new PrizeList();
    ...
    private Prize getPrize(int degree) {
        if (degree < 72) {   // Hammer
            return mPrizeList.getPrize(PrizeList.PRIZE_FIREBALL);
        } else if (degree < 144) {   // Bomb
            return mPrizeList.getPrize(PrizeList.PRIZE_BOMB);
        } else if (degree < 216) {   // 50 coin
            return mPrizeList.getPrize(PrizeList.PRIZE_COIN_50);
        } else if (degree < 288) {   // Gloves
            return mPrizeList.getPrize(PrizeList.PRIZE_COLOR_BALL);
        } else {   // 150 coin
            return mPrizeList.getPrize(PrizeList.PRIZE_COIN_150);
        }
    }
